...
/* ── Botón flotante de WhatsApp — global (wp_footer) para que salga en TODAS las
   páginas (home, VSL, genéricas) cuando esté activado. CSS incluido para no depender
   de main.css (que la home/VSL descargan). ── */
function annabell_whatsapp_float(): void {
    if (!get_theme_mod('show_wa_float', true)) return;
    $raw = (string) get_theme_mod('wa_number', '51XXXXXXXXX');
...
<a href="https://wa.me/<?php echo esc_attr($num); ?>?text=<?php echo $msg; ?>" class="wa-float" target="_blank" rel="noopener" aria-label="<?php echo $tip; ?>" title="<?php echo $tip; ?>">
...
add_action('wp_footer', 'annabell_whatsapp_float');
