...
<?php
$show_wa  = get_theme_mod('show_wa_float', true);
$wa_num   = preg_replace('/[^0-9]/', '', get_theme_mod('wa_number', '51XXXXXXXXX'));
$wa_msg   = rawurlencode(get_theme_mod('wa_message', 'Hola Annabell, vi tu mentoría y quiero saber más 👋'));
$wa_tip   = get_theme_mod('wa_tooltip', '¿Tienes dudas? Escríbeme');
if ($show_wa && $wa_num):
?>
<!-- WhatsApp flotante -->
<a href="https://wa.me/<?php echo esc_attr($wa_num); ?>?text=<?php echo $wa_msg; ?>"
   class="wa-float" target="_blank" rel="noopener"
   aria-label="<?php echo esc_attr($wa_tip); ?>"
   title="<?php echo esc_attr($wa_tip); ?>">
